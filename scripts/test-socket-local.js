...
async function getJoinable() {
  try {
    const res = await axios.get(`${API_BASE}/api/auction/joinable`)
    if (res.data && Array.isArray(res.data) && res.data.length > 0) return res.data[0]
    if (res.data && res.data.items && res.data.items.length > 0) return res.data.items[0].id
    return null
  } catch (err) {
    console.error('Failed to fetch joinable auctions', err.message)
    return null
  }
}
