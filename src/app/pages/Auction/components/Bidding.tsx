...
export default function Bidding() {
    ...
    const { auction, loading, live, reconnecting, countdown, placeBid, pendingBid } = useAuctionLive(auctionId, {
    ...
    const canBid = useMemo(() => {
        ...
        if (!user.role || user.role !== "member") return { ok: false, reason: "Insufficient role" };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (amount < nextMinBid) return toast.error(`Minimum bid is ${formatCurrency(nextMinBid)}`);
    ...
    return (
        <div className="product-bidding-container">
            <div className={`auction-card place-bid-card ${isFinalSecond ? "final-second" : ""}`}>
                <h2 className="card-title">Place Your Bid</h2>

                <div className="current-info">
                    <p>Current Price: <strong>{formatCurrency(currentPrice)}</strong></p>
                    <p>Min Increment: <strong>{formatCurrency(minIncrement)}</strong></p>
                    <p>Next Min Bid: <strong>{formatCurrency(nextMinBid)}</strong></p>
                </div>

                {auction?.status === "ended" ? (
                    <div className="auction-ended">
                        <p>Auction ended. Final price: <strong>{formatCurrency(auction.finalPrice ?? currentPrice)}</strong></p>
                        {auction.winnerId === user?.sub ? (
                            <div>
                                <p className="winner">You won! Complete payment to claim the item.</p>
                                <button className="btn primary">Pay / Complete</button>
                            </div>
                        ) : (
                            <p className="loser">Auction finished. Winner: {auction.winnerId ?? "N/A"}</p>
                        )}
                    </div>
                ) : (
                    <form onSubmit={handleSubmit} className="place-bid-form">
                        <div className="bid-input-wrapper">
                            ...
                            />
                            <p className="bid-helper-text">{!canBid.ok ? canBid.reason : "Your bid will be sent to the server for confirmation."}</p>
                        </div>
                        <button type="submit" className="btn" disabled={!canBid.ok || placing}>
                            {placing ? "Placing..." : pendingBid ? "Bid Pending..." : "Place Bid"}
                        </button>
                    </form>
                )}

                {reconnecting && <p className="reconnect-overlay">Reconnecting... bids are disabled</p>}
                {!live && !loading && <p className="live-status">Not live</p>}
                <div className={`countdown ${isFinalSecond ? "pulse" : ""}`}>Time left: {Math.ceil(countdown / 1000)}s</div>
            </div>
