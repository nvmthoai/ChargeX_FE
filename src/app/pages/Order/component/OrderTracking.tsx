...
export default function OrderTracking({ events }: Props) {
  ...
  if (latestStatus === "cancelled") {
    const cancelledDate = eventMap["cancelled"]
      ? dayjs(eventMap["cancelled"]).format("MMM DD, HH:mm")
      : "--";

    return (
      <div className="rounded-2xl p-6 mb-10 bg-red-50 border border-red-200">
  ...
  const currentIndex = steps.findIndex(
    (s) => s.key === latestStatus?.toLowerCase()
  );
  return (
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.key} className="w-1/4 flex flex-col items-center">
              ...
              >
                <Icon
                  className={`w-5 h-5 ${active ? "text-blue-600" : "text-gray-300"}`}
                />
                {step.label}
              </div>
