...
const auctionService = () => {
    ...
    const sendRequestCreateAuction = useCallback(
        async (sellerId: string, productId: string, note: string) => {
            ...
            try {
                const response = await callApi(HTTP_METHOD.POST, `/auction/request/${sellerId}`, { ...values });
                return response;
            } catch (e: any) {
    ...
    const adminGetRequestCreateAuction = useCallback(
        async () => {
            try {
    ...
    const adminHandleApproveRequest = useCallback(
        async (adminId: string, values: any) => {
            try {
                const response = await callApi(HTTP_METHOD.POST, `/auction/approve/${adminId}`, { ...values });
                return response;
            } catch (e: any) {
                console.log(e?.response?.data);
            }
    ...
    const adminGetAuctions = useCallback(
        async () => {
            try {
